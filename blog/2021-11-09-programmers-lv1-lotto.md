---
slug: '[프로그래머스-lv1]-로또의-최고-순위와-최저-순위'
...
function solution(lottos, win_nums) {
  ...
  const minCount = win_nums.filter((number) => lottos.includes(number)).length;
  const maxCount = minCount + lottos.filter((num) => num === 0).length;
